function [s,d,Trajs,frame] = detectTrajArea6(s,d,Trajs,frame,distances,minArea,maxArea)
...
for dist=distances
for ii=1:nImages-1
    idxArea6 = find(s(ii+1).Area >  minArea & s(ii+1).Area < maxArea & s(ii+1).isTraj==0);  
    for idx=idxArea6
        ...
        if length(index)==6
            [X1,Y1,X2,Y2,X3,Y3,X4,Y4,X5,Y5,X6,Y6] = moveRobots(...
            ...
            s(ii+1).Y(end+1) = Y6;
            %change the area
            s(ii+1).Area(idx) = s(ii+1).Area(idx)/6;
            ...
            Trajs(frame(ii).indexTraj(index(2))).(fn{kk})(end+1) = s(ii+1).(fn{kk})(end-4);
            Trajs(frame(ii).indexTraj(index(3))).(fn{kk})(end+1) = s(ii+1).(fn{kk})(end-3);
            Trajs(frame(ii).indexTraj(index(4))).(fn{kk})(end+1) = s(ii+1).(fn{kk})(end-2);
            Trajs(frame(ii).indexTraj(index(5))).(fn{kk})(end+1) = s(ii+1).(fn{kk})(end-1);
            Trajs(frame(ii).indexTraj(index(6))).(fn{kk})(end+1) = s(ii+1).(fn{kk})(end);
            end
            %fieldname T
            ...
            Trajs(frame(ii).indexTraj(index(2))).P(end+1) = length(s(ii+1).Area)-4;
            Trajs(frame(ii).indexTraj(index(3))).P(end+1) = length(s(ii+1).Area)-3;
            Trajs(frame(ii).indexTraj(index(4))).P(end+1) = length(s(ii+1).Area)-2;
            Trajs(frame(ii).indexTraj(index(5))).P(end+1) = length(s(ii+1).Area)-1;
