function [s,d,Trajs,frame] = detectTrajArea4(s,d,Trajs,frame,distances,minArea,maxArea)
...
for dist=distances
for ii=1:nImages-1
    idxArea4 = find(s(ii+1).Area >  minArea & s(ii+1).Area < maxArea & s(ii+1).isTraj==0);  
    for idx=idxArea4
        ...
        index = find((s(ii+1).X(idx) - s(ii).X(indexPos)).^2 +...
            ...
            < dist^2);
        % if there is 4 particles inside the circle
        if length(index)==4
            [X1,Y1,X2,Y2,X3,Y3,X4,Y4] = moveRobots(...
            ...
            s(ii+1).Y(end+1) = Y4;
            %change the area
            s(ii+1).Area(idx) = s(ii+1).Area(idx)/4;
            ...
            Trajs(frame(ii).indexTraj(index(2))).(fn{kk})(end+1) = s(ii+1).(fn{kk})(end-2);
            Trajs(frame(ii).indexTraj(index(3))).(fn{kk})(end+1) = s(ii+1).(fn{kk})(end-1);
            Trajs(frame(ii).indexTraj(index(4))).(fn{kk})(end+1) = s(ii+1).(fn{kk})(end);
            end
            %fieldname T
            ...
            Trajs(frame(ii).indexTraj(index(2))).P(end+1) = length(s(ii+1).Area)-2;
            Trajs(frame(ii).indexTraj(index(3))).P(end+1) = length(s(ii+1).Area)-1;
            Trajs(frame(ii).indexTraj(index(4))).P(end+1) = length(s(ii+1).Area);
